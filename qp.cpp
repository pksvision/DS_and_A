...
float load_factor()
{
	float lf;int count=0;
	for(int i=0;i<table.size();i++)
	{
		if(table[i]->occupied==true)
			count++;
	}
	return lf=(float)count/(float)table.size();
}

void resize_table()
{
	struct Node * nptr;
	int extra=table.size();
...
void re_fill_table()
{
	...
	for(int i=0;i<table.size();i++)
	{
		table_values.push_back(table[i]->data);
	}
	for(int i=0;i<table.size();i++)
	{
		table[i]->data=MIN;
	...
	{
		if(table_values[i]!=MIN)
		{
			insert(table_values[i]);
		}
	}
}
...
bool search(int value)
{
	...
	int j=1;
	while(j<table.size() && (table[index]->occupied==true || table[index]->deleted==true))
	{
		if(table[index]->data==value)
		{
			return true;
		}
		index=(value+ j*j)%table.size();
		if(index==table.size())
			index=0;
		j++;
	}
