/*
Lowest Common Ancestor Problem in
Binary Search Tree :
Create Node
Find parent node
...
//Vector to store LCA values
std::vector<int> lca_a_b;
...
{
  struct Node * a_ptr=find_NodePtr(a);
  struct Node * b_ptr=find_NodePtr(b);
  struct Node * p;
  std::vector<int>::iterator it;
  lca_a_b.push_back(a_ptr->value);
  p=find_parent(a_ptr);
  while(p!=NULL)
    {
      lca_a_b.push_back(p->value);
      p=find_parent(p);
    }
  for(int i=0;i<lca_a_b.size();i++)
    cout<<lca_a_b[i]<<" ";
  p=b_ptr;
  while(p!=NULL)
    {
      it=find(lca_a_b.begin(),lca_a_b.end(),p->value);
      if(it!=lca_a_b.end())
	break;
      else
	p=find_parent(p);
  ...
  while(option!=6)
      ...
      switch(option)
	{
	   ...
	   case 4: //height of the node
	     ...
	     break;
	   case 5: //lca
	     cout<<"\nEnter two node values to find lca of them\n";
	     int a, b;
	     cin>>a;cin>>b;
	     cout<<"\nLCA of "<<a<<" and "<<b<<" is "<<find_lca(a,b)<<endl;
	     break;
	}
