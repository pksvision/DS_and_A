...
struct Node * deleteFromList(struct Node * nptr)
{
  int value=nptr->data;
  int count=1;
...
struct Node * reverse()
{
	struct Node * new_root,*p,*q,*r;
	new_root=root;
	while(new_root->next!=root)
	{
  ...
  while(opt!=7)
      ...
      switch(opt)
	{
	   ...
	   case 2: cout<<"Enter a value to be deleted from the list"<<endl;
	           ...
	           np=search(val);
	           if(np)
	           		root=deleteFromList(np);
	           	else
	           		cout<<"Value not found. Nothing to dlete"<<endl;
		       break;
	   case 4: printList();
	           break;
	   case 3: if(total>0)
