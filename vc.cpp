...
struct Node * deleteFromList(char value[])
{
  int count=1;
...
void get_element(int pos,char * array)
{
  struct Node * p=root;
  int count=0;
...
void merge(int l, int q, int r)
{
  int n1=q-l+1;
  int n2=r-(q+1)+1;
  char L[n1+1][25], R[n2+1][25];
  for(int i=0;i<n1;i++)
    get_element(l+i,L[i]);
  for(int j=0;j<n2;j++)
    get_element(q+j+1,R[j]);
  strcpy(L[n1],"zzzzzzzzzzzzzzzzzzzzzzzzz");
  strcpy(R[n2],"zzzzzzzzzzzzzzzzzzzzzzzzz");
  int i=0,j=0;
  for(int k=l;k<=r;k++)
    {
      if(strcmp(L[i],R[j])<=0)
